...
export function DashboardShell({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <aside className="fixed inset-y-0 left-0 z-30 w-[260px] border-r border-slate-300 bg-white shadow-sm">
        <div className="flex h-full flex-col">
          <div className="flex items-center gap-3 border-b border-slate-300 px-5 py-4">
          ...
          <nav className="flex-1 space-y-1 px-3 py-4">
            {navItems.filter((item) => !item.adminOnly || user?.authority === "TENANT_ADMIN" || user?.authority === "SYS_ADMIN").map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`flex items-center gap-3 border px-3 py-2 text-sm font-medium transition ${selected ? "border-blue-800 bg-blue-700 text-white" : "border-transparent text-slate-700 hover:border-slate-300 hover:bg-slate-100 hover:text-slate-950"}`}>
          ...
          <div className="border-t border-slate-300 px-5 py-4">
            {isAuthenticated && user ? (
              <div className="space-y-2">
                <div>
                  <p className="text-[11px] font-semibold text-slate-800">{user.firstName || user.email}</p>
                  <p className="mt-1 text-[11px] text-slate-600">{user.authority}</p>
                </div>
                <button type="button" onClick={() => void logout()} className="border border-slate-400 bg-white px-2.5 py-1 text-[11px] font-medium text-slate-800 hover:bg-slate-100">Logout</button>
              </div>
            ) : (
              <div className="space-y-2">
                <p className="text-[11px] font-semibold text-slate-800">Guest</p>
                <p className="mt-1 text-[11px] text-slate-600">Sign in with ThingsBoard user.</p>
                <Link href="/login" className="inline-flex border border-blue-800 bg-blue-700 px-2.5 py-1 text-[11px] font-medium text-white hover:bg-blue-800">Login</Link>
              </div>
            )}
          </div>
        </div>
