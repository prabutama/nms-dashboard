...
import { StatusBadge } from "@/components/nms-ui";
    ...
    <DashboardShell
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={refreshOverview} disabled={refreshing} className="border border-blue-800 bg-blue-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-800 disabled:cursor-wait disabled:border-slate-400 disabled:bg-slate-400">
...
function MiniStat({ title, value, note, status }: { title: string; value: string | number; note?: string; status?: string }) {
  return (
    <div className={`border border-slate-300 bg-white px-3 py-2 shadow-sm ${status ? `border-l-4 ${statusBorderClass(status)}` : ""}`}>
...
function Panel({ title, action, children }: { title: string; action?: ReactNode; children: ReactNode }) {
  return (
    <div className="min-h-0 border border-slate-300 bg-white shadow-sm">
