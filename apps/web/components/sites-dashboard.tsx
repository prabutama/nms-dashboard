  ...
  return (
    <DashboardShell title="Sites" subtitle="Monitored locations and site-level inventory.">
      ...
      <div className="border border-slate-300 bg-white shadow-sm">
        ...
        {sitesQuery.isLoading || reportSitesQuery.isLoading ? <p className="px-4 py-5 text-xs text-slate-600">Loading sites...</p> : null}
        ...
        <div className="divide-y divide-slate-200">
          {(sitesQuery.data?.items || []).map((site) => {
            ...
            return (
              <Link key={site.assetId} href={`/sites/${site.siteKey}`} className="flex items-center justify-between gap-4 px-4 py-3 transition hover:bg-slate-100">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-slate-950">{site.name}</p>
                  <p className="truncate text-xs text-slate-600">{site.type}</p>
                </div>
